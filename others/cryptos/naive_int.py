# naive interpreter that extracts the non used lines from meas.out file
# according if the value of each line passes or not a certain
# threshold given by the variable non_used_line_value
# This is not the best approach, since it only needs 1x a true positive
# not under that threshold limit to be discarded as possible key by the program
...
def round_1_attack():
    ...
    while(True):
        ...
        for j in range(0,16):
            for hki in range(0,256):
                hacc = p[j] ^ hki
                line_hacc = (tables[j%4] + (hacc//delta)) % 64
    ...
    for i , ki in enumerate (k):
        ...
        if counter == 16:
            ...
            print("key byte:" + str(i) + " has multiple ~<ki>")
    print(part_key)
...
def round_2_attack():
            ...
            for low_hkB in range(0, 16):
                for low_hkC in range(0, 16):
                    for low_hkD in range(0, 16):
                        ...
                        #original 1-Round tromer equations
    ...
    for lk_index, lk_item in enumerate(lk_list):
        if len(lk_item) == 1:
            set_final_key(lk_index,lk_item[0])
